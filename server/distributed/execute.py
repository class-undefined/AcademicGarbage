...
config = yaml.load(open(
    f"{EnvConfig.WORK_SPACE}/server/distributed/config.yaml", "rb"), yaml.SafeLoader)
config["YOLOV5_PT"] = EnvConfig.WORK_SPACE + config["YOLOV5_PT"]
config["YOLOV5_DETECT"] = EnvConfig.WORK_SPACE + config["YOLOV5_DETECT"]
    ...
    with open(path, "r", encoding="utf8") as f:
        while True:
            ...
            if line == "":
                break
            cols = line.split(" ")
            bbox = [float(cols[1]), float(cols[2]),
...
def identify(url: str, envs: Union[Dict, None] = None):
    ...
    import os
    YOLOV5_PT = os.getenv(
        "YOLOV5_PT") or envs is not None and envs["YOLOV5_PT"]
    YOLOV5_DETECT = os.getenv(
        "YOLOV5_DETECT") or envs is not None and envs["YOLOV5_DETECT"]
    YOLOV5_DETECT_ARGS = os.getenv(
        "YOLOV5_DETECT_ARGS") or envs is not None and envs["YOLOV5_DETECT_ARGS"]
...
def upload(user_id: str, photo_id: str, original_url: str, envs: Union[Dict, None] = None):
    ...
    import requests
    import json
    url = f"http://127.0.0.1:5001/middle/identify_result"
...
class worker:
    def __init__(self, user_id: str, photo_id: str, original_url: str, config: Dict):
    ...
    def __call__(self):
        upload(self.user_id, self.photo_id, self.original_url, self.config)
...
def wrap_identify(user_id: str, photo_id: str, original_url: str):
    ...
    p = multiprocessing.Process(target=worker(
        user_id, photo_id, original_url, config))
    p.start()
